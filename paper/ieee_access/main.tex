...
\begin{abstract}
Hybrid architectures that correct a gradient-boosted prior with a neural residual head are widely reported to improve tabular property prediction in materials informatics. Evaluating one such architecture for band gaps of two-dimensional materials under a protocol that closes six leakage pathways, we find that its reported advantage does not survive: it fails to improve on the prior it corrects ($R^2 = 0.810 \pm 0.047$ against $0.823 \pm 0.043$; superior on 12 of 25 fold estimates; $p = 0.27$), and none of its physics-motivated components contributes measurably. A component ablation traces the discrepancy to how the tree prior is constructed, a detail invisible in the metrics such comparisons normally report. What does work on the same 1{,}169 rectangular-lattice C2DB materials is heterogeneous stacking: an ensemble of five base learners is the strongest configuration tested ($R^2 = 0.880 \pm 0.037$, MAE $0.360$ eV), beating every tree baseline on 8 of 8 independent held-out sets. Because its margin over the best single model is small, we justify it on decision-relevant grounds rather than accuracy alone. The identity of the best single model is unstable across folds, so committing to one in advance carries a risk the ensemble removes: it beats a committed choice on 15 of 15 folds, cuts predictions in error by more than 1 eV from 7.3\% to 6.1\%, and trains in 8 seconds against the CPU-hours of the calculation it replaces. Decomposing accuracy by descriptor cost, we find that removing every DFT-derived descriptor costs $0.044$ in $R^2$, most of the attainable accuracy resting on composition and symmetry.
\end{abstract}
...
All data derive from one database and one Bravais class, with 1{,}169 labelled materials. In the DFT-free tier the ensemble's advantage over a well-tuned XGBoost is not statistically significant ($p = 0.097$). Single-split held-out comparisons at this sample size lack the resolution to separate closely matched models, which is why Section~\ref{sec:holdout} reports eight independent held-out sets rather than one. The uncertainty intervals are reported as diagnosed, not repaired.
