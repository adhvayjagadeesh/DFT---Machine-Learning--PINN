...
\begin{abstract}
Hybrid architectures that correct a gradient-boosted prior with a neural residual head are widely reported to improve tabular property prediction in materials informatics. Evaluating one such architecture for band gaps of two-dimensional materials under a protocol that closes six leakage pathways, we find that its reported advantage does not survive: it fails to improve on the prior it corrects ($R^2 = 0.810 \pm 0.047$ against $0.823 \pm 0.043$; superior on 12 of 25 fold estimates; $p = 0.54$), and none of its physics-motivated components contributes measurably. A component ablation traces the discrepancy to how the tree prior is constructed, a detail invisible in the metrics such comparisons normally report. What does work on the same 1{,}169 rectangular-lattice C2DB materials is heterogeneous stacking: an ensemble of five base learners is the strongest configuration tested ($R^2 = 0.877 \pm 0.038$, MAE $0.363$ eV), beating the gradient-boosting baseline on 8 of 8 independent held-out sets ($p = 0.002$). Because its margin over the best single model is small, we justify it on decision-relevant grounds rather than accuracy alone. The identity of the best single model is unstable across folds, so committing to one in advance carries a risk the ensemble removes: it beats a committed choice on 22 of 25 folds, cuts predictions in error by more than 1 eV from 7.4\% to 6.3\%, and trains in 8 seconds against the CPU-hours of the calculation it replaces. Decomposing accuracy by descriptor cost, we find that removing every DFT-derived descriptor costs $0.044$ in $R^2$, most of the attainable accuracy resting on composition and symmetry.
\end{abstract}
...
No physics-motivated component contributes positively, and the tree prior alone is the strongest configuration in the table (Fig.~\ref{fig:ablation}). The second row identifies the mechanism. Reverting to an in-sample prior \emph{raises} measured $R^2$, from $0.801$ to $0.817$, recovering almost the whole gap to the tree prior alone ($0.819$). It does not exceed that prior, and we do not claim it reproduces the original gain over the baseline; what it reproduces is the corrector's degeneration into a pass-through. With a prior that appears near-perfect on training rows the correction head has almost no residual signal to fit, so it learns to emit approximately zero and the architecture collapses onto the tree it was meant to improve. The ablation recovers that behaviour on demand by changing one line, which is why we regard the effect as a property of the evaluation rather than of the model.
...
\label{eq:coverage}
\end{equation}
should equal the nominal level of $0.50$; the measured value is $C = 0.172$ at a mean interval width of $2.47$ eV (Fig.~\ref{fig:calibration}). The intervals are simultaneously wide and badly miscalibrated, and the deficit persists across every nominal level tested (Fig.~\ref{fig:calibration}(a)).
...
\midrule
All descriptors      & $0.877 \pm 0.036$ & $0.364$ & --- \\
...
Fig.~\ref{fig:contributions} contrasts the fitted ensemble weights with a drop-one ablation. The two disagree: SVR receives zero weight yet gradient boosting receives $0.238$ while contributing nothing measurable when removed. Collinear members split weight between them, so weights alone cannot establish a contribution and the drop-one test is the appropriate instrument. That test orders the members consistently with their individual accuracy --- removing the PINN costs most ($+0.008$ eV$^2$, 17 of 25 estimates degraded), then the MLP and random forest --- but no single member's contribution is statistically significant under the corrected two-sided test ($p = 0.22$--$0.94$). The defensible statement is that the ensemble as a whole outperforms its members, not that any one member is indispensable.
