...
class QuantileSplittingHybridPINN(nn.Module):
    """
    INNOVATION: Quantile Residual Splitting Network
    Predicts multi-scale variance boundaries (25th, 50th, 75th percentiles) 
    to isolate and correct highly irregular non-linear tree residuals.
    """
    def __init__(self, raw_dim):
        super(QuantileSplittingHybridPINN, self).__init__()
        ...
        # Outputs 3 heads: [Lower Quantile, Median Correction, Upper Quantile]
        self.quantile_heads = nn.Linear(64, 3)
    ...
    def forward(self, x_hybrid):
        ...
        x_rep = self.input_layer(x_base_features)
        x_comp = self.hidden_layer(x_rep)
        quantiles = self.quantile_heads(x_comp)
        
        q25, q50, q75 = quantiles[:, 0:1], quantiles[:, 1:2], quantiles[:, 2:3]
        
        # Dynamic fusion weighted by the uncertainty bounds (interval spread)
        uncertainty_weight = torch.sigmoid(q75 - q25)
        residual_correction = q50 + 0.1 * uncertainty_weight * (q75 + q25)
        
        return tree_shortcut + residual_correction, quantiles

def quantile_physics_loss_fn(y_pred, quantiles, y_true):
    """Combined Pinball (Quantile) Loss and Gated Physics Penalty."""
    q25, q50, q75 = quantiles[:, 0:1], quantiles[:, 1:2], quantiles[:, 2:3]
    residual = y_true - (q50) # target residual baseline focus
    
    # Standard Pinball loss calculations for structural coverage
    loss_q25 = torch.max(0.25 * residual, (0.25 - 1.0) * residual).mean()
    loss_q75 = torch.max(0.75 * residual, (0.75 - 1.0) * residual).mean()
    
    # Boundary-aware physics buffer check
    base_violation = torch.relu(-y_pred)
    boundary_buffer = torch.exp(torch.relu(0.05 - y_pred)) - 1.0
    physics_penalty = torch.mean(base_violation + 0.1 * boundary_buffer)
    
    return 0.5 * (loss_q25 + loss_q75) + physics_penalty
...
print("Starting Quantile Residual Splitting Benchmarking Suite...")
...
for fold, (train_idx, test_idx) in enumerate(kf.split(X)):
    ...
    for epoch in range(1500):
        ...
        # Standard functional boundary physics tracking
        base_violation = torch.relu(-y_pred)
        boundary_buffer = torch.exp(torch.relu(0.05 - y_pred)) - 1.0
        p_loss = torch.mean(base_violation + 0.1 * boundary_buffer)
        
        total_pinn_loss = mse_loss(y_pred, y_train_tensor) + 0.001 * p_loss
        total_pinn_loss.backward()
        pinn_optimizer.step()
        pinn_scheduler.step()
        
    # 6. Hybrid Model A: Quantile RF-PINN
    ...
    hybrid_rf = QuantileSplittingHybridPINN(raw_dim=X_train_scaled.shape[1])
    ...
    for epoch in range(1500):
        ...
        pred, q_vectors = hybrid_rf(torch.tensor(X_train_hybrid_rf, dtype=torch.float32))
        loss_data = mse_loss(pred, y_train_tensor)
        loss_phys_q = quantile_physics_loss_fn(pred, q_vectors, y_train_tensor)
        with torch.no_grad(): adaptive_w = 0.001 * (loss_data.item() + 1e-5)
        total_loss = loss_data + adaptive_w * loss_phys_q
        total_loss.backward()
        optimizer_rf.step()
        scheduler_rf.step()

    # 7. Hybrid Model B: Quantile GBR-PINN
    ...
    hybrid_gbr = QuantileSplittingHybridPINN(raw_dim=X_train_scaled.shape[1])
    ...
    for epoch in range(1500):
        ...
        pred, q_vectors = hybrid_gbr(torch.tensor(X_train_hybrid_gbr, dtype=torch.float32))
        loss_data = mse_loss(pred, y_train_tensor)
        loss_phys_q = quantile_physics_loss_fn(pred, q_vectors, y_train_tensor)
        with torch.no_grad(): adaptive_w = 0.001 * (loss_data.item() + 1e-5)
        total_loss = loss_data + adaptive_w * loss_phys_q
    ...
    with torch.no_grad():
        ...
        preds['hybrid_rf'], _ = hybrid_rf(torch.tensor(X_test_hybrid_rf, dtype=torch.float32))
        preds['hybrid_rf'] = scaler_y.inverse_transform(preds['hybrid_rf'].numpy())
        preds['hybrid_gbr'], _ = hybrid_gbr(torch.tensor(X_test_hybrid_gbr, dtype=torch.float32))
        preds['hybrid_gbr'] = scaler_y.inverse_transform(preds['hybrid_gbr'].numpy())
...
print("\n--- Significance Verification (Quantile Hybrid GBR vs. Standalone GBR) ---")
