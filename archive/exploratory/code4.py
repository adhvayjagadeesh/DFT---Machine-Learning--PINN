...
class ResidualHybridPINN(nn.Module):
    def __init__(self, input_dim):
        ...
        # input_dim matches the total stacked hybrid vector space
    ...
    def forward(self, x_hybrid):
        ...
        tree_shortcut = x_hybrid[:, -1:] # Can be RF or GBR shortcut dynamically
...
# Initializing tracking lists for all 7 models
metrics = {
    'rf':          {'r2': [], 'mse': []},
    'gbr':         {'r2': [], 'mse': []},
    'svr':         {'r2': [], 'mse': []},
...
# Tracking squared errors globally for significance mapping
all_gbr_sq_errors = []
all_hybrid_gbr_sq_errors = []

print("Starting Comprehensive 7-Model Benchmarking Suite (with GBR-PINN)...")
...
for fold, (train_idx, test_idx) in enumerate(kf.split(X)):
    ...
    for epoch in range(1200):
        ...
        mlp_pred = mlp_model(X_train_tensor)
        loss = mse_loss(mlp_pred, y_train_tensor)
        loss.backward()
        mlp_optimizer.step()
    ...
    hybrid_rf = ResidualHybridPINN(input_dim=X_train_hybrid_rf.shape[1])
    ...
    for epoch in range(1500):
        ...
        total_loss = loss_data + adaptive_w * physics_loss_fn(pred)
        total_loss.backward()
        optimizer_rf.step()
        scheduler_rf.step()

    # 7. Hybrid Model B: GBR-PINN (Your New Request)
    ...
    hybrid_gbr = ResidualHybridPINN(input_dim=X_train_hybrid_gbr.shape[1])
